...
Window startMultiplayerModeWindow()
{
    // initialization   
    ...
    Game*game = initGame(); //init game
    game->mode = 0; //set mode as 0 for multiplayer
    Score*score = calloc(1,sizeof(Score)); // initialize score. calloc to avoid garbage value which may cause segmentation fault.
    // initialize background, board and player turn
        ...
        {   
            ...
            case SDL_MOUSEBUTTONDOWN:
                ...
                if(isMouseHoverOver(mousePosition.x,mousePosition.y,&backBtn.button) && backBtn.state == MAIN_MENU)
                    game->state = QUIT; //if user clicks on return button to go back to main menu
